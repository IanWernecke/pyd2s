...
def create_checksum(binary_data, offset=12):
	...
	on_checksum = checksum(
		numpy.zeros(checksum_byte_length, dtype=numpy.uint8),
...
class Game(object):
	...
	def from_bytes(self, binary):
		...
		self.merc_dead, self.merc_id, self.merc_name_id, self.merc_type, self.merc_exp = struct.unpack('<HIHHI', bio.read(14))
	...
	def reset_hephaesto(self):
		...
		for byte_index in (256,):
			...
			self.quests = self.quests[:byte_index] + first_byte.to_bytes(1, 'little') + \
				second_byte.to_bytes(1, 'little') + self.quests[byte_index + 2:]
